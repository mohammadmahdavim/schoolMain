...
class SelectionController extends Controller
{
    public function index()
    {
        ...
        $selections = Selection::whereIn('id', $memeber)
            ...
            ->with('item')
            ->get();
        return view('Teacher.selection.index', ['rows' => $selections]);
    }
    ...
    public function past()
    {
        ...
        $selections = Selection::whereIn('id', $memeber)
            ...
            ->with('item')
            ->get();
        return view('Teacher.selection.past', ['rows' => $selections]);
    }
    ...
    public function sabt($id)
    {
        $selection = Selection::where('id', $id)
            ->with('option')
            ->first();
        return view('Teacher.selection.sabt', ['selection' => $selection]);
    ...
    public function view($id){
        ...
        $row = SelectionOption::where('Selection_id', $id)
            ...
            ->orderByDesc('value')
            ->get();
        return view('Teacher.selection.optionview', ['row' => $row]);
    }
}
