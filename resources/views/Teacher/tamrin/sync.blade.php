...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="/teacher/archive/sync/{{$item->id}}">
                {{csrf_field()}}
                <input name="model" value="Tamrin" hidden>
                    ...
                    <div class="multi-select-full">
                        ...
                        <select name="class[]" class="multiselect-warning"
                            ...
                            @foreach($allclass as $allclas)
                                <option value="{{$allclas->class_id}}">{{$allclas->class_id}}کلاس</option>
                            @endforeach
                        </select>
                        <select name="users[]" class="multiselect-info"
                            ...
                            @foreach($allstudents as $allstudent)
                                <option value="{{$allstudent->id}}"
                                        @if(in_array($allstudent->id , $syncs->pluck('user_id')->toArray())) selected @endif>{{$allstudent->f_name}} {{$allstudent->l_name}}- کلاس {{$allstudent->class}}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
